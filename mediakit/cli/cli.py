...
class DownloadCLI:
    ...
    def show_loading_label(self):
        ...
        number_of_dots_displayed = 0

        def run_loading_animation():
            nonlocal number_of_dots_displayed

            while self.is_loading:
                number_of_dots_displayed = (number_of_dots_displayed + 1) % 4
                ...
                screen.update_content(
                    self.loading_label,
                    "\nLoading video" + "." * number_of_dots_displayed + "\n\n",
                )
    ...
    def register_download_info(self, video, output_path, filename, formats):
        ...
        self.media_resources_to_download = self._get_media_resources_to_download(
            formats
        )
    ...
    def show_video_heading(self):
        ...
        formatted_video_title = self._format_video_title(remaining_number_of_characters)
    ...
    def show_download_summary(self):
        ...
        is_preceded_by_format_warnings = (
            len(self.skipped_formats) > 0 or len(self.formats_replaced_by_fallback) > 0
        )
    ...
    def ask_for_confirmation_to_download(self):
        ...
        answer, prompt_message = screen.prompt(
            prompt_message, valid_inputs=["", "y", "n"]
        )
    ...
    def update_download_progress_info(self, bytes_remaining):
        ...
        if self.downloading_media_resource.output_type == "videoaudio":
            if downloading_stream is self.downloading_media_resource.video:
                self.downloading_media_resource.video_bytes_remaining = bytes_remaining
            elif downloading_stream is self.downloading_media_resource.audio:
                self.downloading_media_resource.audio_bytes_remaining = bytes_remaining
    ...
    def were_all_formats_skipped(self):
        return (
            len(self.media_resources_to_download) == 0 and len(self.skipped_formats) > 0
        )
    ...
    def _is_definition_available(self, base_format, definition):
        if base_format.startswith("video"):
            ...
            video_streams_with_specified_resolution = self.video.streams.filter(
            ...
            audio_streams_with_specified_resolution = self.video.streams.filter(
    ...
    def _update_download_progress_ui(self):
        screen.update_content(
            self.download_progress_ui, self._get_current_download_progress()
        )
    ...
    def _update_progress_ui_interval_if_necessary(self):
        ...
        required_interval = self.ui_update_intervals.get(
            download_status, self.default_ui_update_interval
        )
    ...
    def _get_current_download_progress(self):
        ...
        status_character, status_color = self._get_status_resources(download_status)
    ...
    def _get_download_progress_heading(
        self, download_status, status_character, status_color
    ):
        label = download_status.title()

        is_downloading_first_media_resource = (
            self.downloading_media_resource is self.media_resources_to_download[0]
        )
        ...
        heading = (
            ...
            + colored(f"{self.short_video_title} ", style=Colors.style.BRIGHT)
            + colored(
    ...
    def _get_status_resources(self, download_status):
        ...
        if download_status == DownloadStatusCodes.DOWNLOADING:
            self.rotating_line_frame = (self.rotating_line_frame + 1) % 4
        ...
        if download_status == DownloadStatusCodes.CONVERTING:
            self.loading_dots_frame = (self.loading_dots_frame + 1) % 10
    ...
    def _show_skipped_formats_warning(self):
        formatted_skipped_formats = [
            f'[{" ".join(skipped_format)}]' for skipped_format in self.skipped_formats
        ]
        ...
        screen.append_content(skipped_formats_message, ContentCategories.WARNING)
    ...
    def _show_fallback_replacement_summary(self):
        ...
        for replaced_format in self.formats_replaced_by_fallback:
            ...
            formatted_base = get_formatted_replaced_format(base_format, base_definition)
